...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class FlipRAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, };

        protected FlipRAnimator(in Guid ahandle, bool transfer)
        ...
        internal static FlipRAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new FlipRAnimator(handle, transfer: t));

            return result as FlipRAnimator;
        }
    }
}