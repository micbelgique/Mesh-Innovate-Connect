...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class PlayVideoAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, };

        protected PlayVideoAnimator(in Guid ahandle, bool transfer)
        ...
        internal static PlayVideoAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new PlayVideoAnimator(handle, transfer: t));

            return result as PlayVideoAnimator;
        }
    }
}