...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class TpToEventZoneAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, };

        protected TpToEventZoneAnimator(in Guid ahandle, bool transfer)
        ...
        internal static TpToEventZoneAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new TpToEventZoneAnimator(handle, transfer: t));

            return result as TpToEventZoneAnimator;
        }
    }
}