...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class OpenandcloseSlideAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, };

        protected OpenandcloseSlideAnimator(in Guid ahandle, bool transfer)
        ...
        internal static OpenandcloseSlideAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new OpenandcloseSlideAnimator(handle, transfer: t));

            return result as OpenandcloseSlideAnimator;
        }
    }
}