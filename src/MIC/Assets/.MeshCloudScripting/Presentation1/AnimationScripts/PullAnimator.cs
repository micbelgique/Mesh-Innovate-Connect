...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class PullAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, };

        protected PullAnimator(in Guid ahandle, bool transfer)
        ...
        internal static PullAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new PullAnimator(handle, transfer: t));

            return result as PullAnimator;
        }
    }
}