...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class SimuConfAnimatorAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, };

        protected SimuConfAnimatorAnimator(in Guid ahandle, bool transfer)
        ...
        internal static SimuConfAnimatorAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new SimuConfAnimatorAnimator(handle, transfer: t));

            return result as SimuConfAnimatorAnimator;
        }
    }
}