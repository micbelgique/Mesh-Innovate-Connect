...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class Openandclose1Animator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, 1F, };

        protected Openandclose1Animator(in Guid ahandle, bool transfer)
        ...
        internal static Openandclose1Animator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new Openandclose1Animator(handle, transfer: t));

            return result as Openandclose1Animator;
        }
    }
}