...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class OpenandclosewindowAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, };

        protected OpenandclosewindowAnimator(in Guid ahandle, bool transfer)
        ...
        internal static OpenandclosewindowAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new OpenandclosewindowAnimator(handle, transfer: t));

            return result as OpenandclosewindowAnimator;
        }
    }
}