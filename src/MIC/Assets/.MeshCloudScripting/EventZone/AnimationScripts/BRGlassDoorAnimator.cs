...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class BRGlassDoorAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, };

        protected BRGlassDoorAnimator(in Guid ahandle, bool transfer)
        ...
        internal static BRGlassDoorAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new BRGlassDoorAnimator(handle, transfer: t));

            return result as BRGlassDoorAnimator;
        }
    }
}