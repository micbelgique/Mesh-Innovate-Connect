...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class OvenFlipAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { 1F, 1F, 1F, };

        protected OvenFlipAnimator(in Guid ahandle, bool transfer)
        ...
        internal static OvenFlipAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateCloudObject(
                cookie,
                transfer,
                (handle, t) => new OvenFlipAnimator(handle, transfer: t));

            return result as OvenFlipAnimator;
        }
    }
}